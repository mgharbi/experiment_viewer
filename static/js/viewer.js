function TwoFace(id, width, height) {
    ...
    // var canvasWidth = maxSz;
    // var canvasHeight = canvasWidth*aspect;
    //
    var canvasHeight = maxSz;
    var canvasWidth = canvasHeight/aspect;
    
    var default_scale = canvasWidth/width;
    ...
    Object.defineProperty(this, 'imwidth', {
        get: function() {
            return width;
        }
    });

    Object.defineProperty(this, 'imheight', {
        get: function() {
            return height;
        }
    });
    ...
    this.lastX = canvasWidth/2;
    this.lastY = canvasHeight/2;
...
TwoFace.prototype = {
    ...
    drawImages: function(ctx, before, after) {
        ...
        var start_x = this.width/2-this.imwidth/2;
        var start_y = this.height/2-this.imheight/2

        split = (split>start_x)? split-start_x : 1;
        ctx.drawImage(after, start_x, start_y,  this.imwidth, this.imheight);
        ctx.drawImage(before, 0, 0, split, this.imheight, start_x, start_y, split, this.imheight);
    },
    ...
    drawHandle: function(ctx) {
        ...
        ctx.fillRect(split - 1, -this.height/2, 4, 2*this.height);
    },

    mousedown: function(event) {
        if(!event.shiftKey) {
            // var divide = event._x / this.width;
            var divide =  (event.offsetX)/ this.width;
            this.divide = divide;
        }
    ...
    wheel: function(event) {
        var delta = event.deltaY ? event.deltaY : 0;
        var factor = Math.pow(this.scaleFactor, delta/20.0);
